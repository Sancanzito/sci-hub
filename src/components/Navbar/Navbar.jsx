...
// ==========================================
// 3. UNIT CONVERTER (Visual Layout for Preview)
// ==========================================
const UnitConverter = () => {
  const [selectedCategory, setSelectedCategory] = useState('length');
  const categories = ['length', 'mass', 'volume', 'temperature', 'time', 'speed'];
  ...
  return (
    <div className="p-4 space-y-4 bg-transparent">
      <div className="text-center">
        <h3 className="text-sm font-bold text-slate-800 dark:text-cyan-50">
          Universal Unit Converter
        </h3>
        <p className="text-xs font-medium text-slate-500 dark:text-cyan-200/60">
          Convert between any units (Preview Layout)
        </p>
      </div>

      <div>
        <label className="text-[10px] font-bold text-slate-600 dark:text-cyan-200/80 block mb-2 uppercase tracking-wider">
          Category
        </label>
        <div className="grid grid-cols-3 gap-1.5 max-h-32 overflow-y-auto p-1 scrollbar-thin scrollbar-thumb-cyan-500/20">
          {categories.map((cat) => {
            const config = categoryConfig[cat] || { name: cat, icon: '🔧', color: 'from-gray-500 to-gray-600' };
            return (
              <motion.button
                key={cat}
                ...
                whileTap={{ scale: 0.98 }}
                className={`text-xs px-2 py-1.5 rounded-lg transition-all flex items-center justify-center gap-1 border ${
                  selectedCategory === cat
                    ? `bg-gradient-to-r ${config.color} text-white shadow-md border-transparent`
                    : 'bg-white/40 dark:bg-black/20 text-slate-700 dark:text-cyan-100/80 hover:bg-white/60 dark:hover:bg-cyan-900/30 border-white/50 dark:border-cyan-500/20 backdrop-blur-sm'
                }`}
              >
      ...
      <div className="text-center pt-2">
        <p className="text-[9px] font-semibold tracking-wider text-slate-400 dark:text-cyan-200/40 uppercase">
          Powered by convert-units • Supports 13+ categories
        </p>
      </div>
    </div>
  );
          ...
          >
            <div>
              <button
                ...
                className="w-full flex items-center justify-between p-4 hover:bg-white/50 dark:hover:bg-cyan-900/30 transition-colors"
              >
                <div className="flex items-center gap-3">
                  <span className="text-2xl drop-shadow-sm">📐</span>
                  <div className="text-left">
                    <div className="text-sm font-bold text-slate-800 dark:text-cyan-50">Unit Converter</div>
                    <div className="text-xs font-medium text-slate-500 dark:text-cyan-200/60">Convert between different units</div>
                  </div>
                </div>
                <motion.span
                  animate={{ rotate: isUnitConverterOpen ? 180 : 0 }}
                  className="text-slate-400 dark:text-cyan-500/50"
                >
                  ▼
                </motion.span>
              </button>
              ...
              <AnimatePresence>
                {isUnitConverterOpen && (
                  <motion.div
                    ...
                    className="border-t border-white/40 dark:border-cyan-500/20"
                  >
                    <UnitConverter />
                  </motion.div>
                )}
            ...
            <div className="h-px bg-gradient-to-r from-transparent via-white/50 dark:via-cyan-500/20 to-transparent" />
